...
public class MemoWidget extends AppWidgetProvider {
 ...
 static void update(Context c,AppWidgetManager am,int id){RemoteViews v=new RemoteViews(c.getPackageName(),R.layout.widget);int n=MemoStore.pendingToday(c);v.setTextViewText(R.id.widgetSummary,"未完了 "+n+"件");List<String> ts=MemoStore.times(c);v.setTextViewText(R.id.widgetNext,"通知: "+String.join("・",ts));StringBuilder s=new StringBuilder();for(MemoStore.Memo x:MemoStore.memos(c)){if(x.day.equals(MemoStore.today())&&!x.done){if(s.length()>0)s.append("\n");s.append("・").append(x.text);if(s.toString().split("\n").length>=3)break;}}v.setTextViewText(R.id.widgetMemos,s.length()==0?"今日のメモはありません":s.toString());Intent i=new Intent(c,MainActivity.class).putExtra("speak",true);PendingIntent p=PendingIntent.getActivity(c,22,i,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);v.setOnClickPendingIntent(R.id.widgetRecord,p);am.updateAppWidget(id,v);}
